...
public class ItemShape extends Item {
    ...
    public ItemShape(String str) {
        ...
        for (int i = 0; i < 4; i++) { // fait uniquement pour la première couche
            switch (str.charAt(i * 2)) {
                case 'C':
                    tabSubShapes[i] = SubShape.Carre;
                    break;
                case '-':
                    tabSubShapes[i] = SubShape.None;
                    break;
                case 'O':
                    tabSubShapes[i] = SubShape.Circle;
                    break;
                default:
            ...
            switch (str.charAt((i * 2 + 1))) {
                case 'r':
                    tabColors[i] = Color.Red;
                    break;
                case 'b':
                    tabColors[i] = Color.White;
                    break;
                case '-':
                    tabColors[i] = null;
                    break;
    ...
    // TODO : écrire l'ensemble des fonctions de transformation souhaitées, définir
